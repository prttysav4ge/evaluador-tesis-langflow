...
Transformaciones:
  1. Los nodos OpenAIModel dejan de llevar la OPENAI_API_KEY en texto plano: el
     campo api_key pasa a load_from_db=True con value="OPENAI_API_KEY", de modo
     que Langflow la resuelve desde una VARIABLE GLOBAL homónima. Esa variable se
     crea desde el entorno del Space mediante
     LANGFLOW_VARIABLES_TO_GET_FROM_ENVIRONMENT=OPENAI_API_KEY (ver Dockerfile).
...
OPENAI_VAR = "OPENAI_API_KEY"  # nombre de la variable global en Langflow
...
print(f"  nodos OpenAI sin clave literal (usan var global): {stripped}")
